...
- (void)updateIndicators {
    ...
    } else if (mode == MBProgressHUDModeTextOnly) {
        self.indicator = nil;
    }
    else {
        if (mode == MBProgressHUDModeIndeterminateLarge) {
...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view text:(NSString *)text mode:(MBProgressHUDMode)mode animated:(BOOL)animated {
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
    hud.labelText = text;
    hud.mode = mode;
	[view addSubview:hud];
...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view text:(NSString *)text mode:(MBProgressHUDMode)mode animated:(BOOL)animated hideAfter:(NSTimeInterval)timeInterval {
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
    hud.labelText = text;
    hud.mode = mode;
	[view addSubview:hud];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated completion:(void(^)())completion {
	UIView *viewToRemove = nil;
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[MBProgressHUD class]]) {
			viewToRemove = v;
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.mode = MBProgressHUDModeIndeterminateLarge;
        self.labelText = nil;
...
- (void)layoutSubviews {
    ...
    self.height = indFrame.size.height + margin;
    ...
    if (nil != self.labelText) {
        ...
        if (nil != self.detailsLabelText) {
            ...
            lFrame.origin.y -= ((floorf(lHeight / 2 + PADDING / 2)) - 2.f);
            label.frame = lFrame;
...
- (void)hide:(BOOL)animated completion:(void(^)())completion {
    ...
    [self hideUsingAnimation:useAnimation completion:completion];
}
...
- (void)hideUsingAnimation:(BOOL)animated completion:(void(^)())completion {
    // Fade out
    if (animated) {
        [UIView animateWithDuration:.35 animations:^{
...
#define MB_RADIANS(degrees) ((degrees * (float)M_PI) / 180.0f)
...
- (void)setTransformForCurrentOrientation:(BOOL)animated {
	...
	rotationTransform = CGAffineTransformMakeRotation(MB_RADIANS(degrees));
